  ...
  return (
    ...
    >
      <Notes
        ...
        content={[
          "You might already by familiar with GraphQL and Apollo Client, but for those of you who are not, here’s how you can perform the exact same user fetching I showed you with these tools",
          "And as we can see, by using this technique, we don’t only get access to a more convenient code - imo - we also have access to more features, like ```loading``` and ```error``` for instance.",
          "I took Apollo as an example but, the same thing applies with the ```<QueryRenderer />``` component from Relay!",
